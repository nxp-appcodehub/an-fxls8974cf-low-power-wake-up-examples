...
/**
 * @file mcxn9xxevk.c
 * @brief The mcxn9xxevk.c file defines GPIO pins and I2C CMSIS utilities for MCX-9XX-EVK board.
 */

#include "mcxn9xxevk.h"
...
gpioHandleKSDK_t RED_LED = {
    .base = GPIO3, .pinNumber = 4, .mask = 1 << (4), .clockName = kCLOCK_Gpio3, .portNumber = 3};
gpioHandleKSDK_t GREEN_LED = {
    .base = GPIO3, .pinNumber = 2, .mask = 1 << (2), .clockName = kCLOCK_Gpio3, .portNumber = 3};
gpioHandleKSDK_t BLUE_LED = {
    .base = GPIO3, .pinNumber = 3, .mask = 1 << (3), .clockName = kCLOCK_Gpio3, .portNumber = 3};
